...
int main(string[] args)
{
    ...
    EnvData envData;
    envData.all_args      = "-inline -release -g -O -unittest";//getenv("ARGS");
    envData.results_dir   = r"test_results";//getenv("RESULTS_DIR");
    envData.sep           = std.path.dirSeparator;//getenv("SEP");
    envData.dsep          = getenv("DSEP");
    envData.obj           = ".obj";//getenv("OBJ");
    envData.exe           = ".exe";//getenv("EXE");
    envData.os            = "win32";//getenv("OS");
    envData.dmd           = "dmd";//replace(getenv("DMD"), "/", envData.sep);
    envData.compiler      = "dmd"; //should be replaced for other compilers
    envData.ccompiler     = getenv("CC");
    envData.model         = "32";//getenv("MODEL");
    ...
    foreach(i, c; combinations(testArgs.permuteArgs))
    {
        ...
        //writeln("test_app_dmd = ", test_app_dmd);
