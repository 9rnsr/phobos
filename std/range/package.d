...
if (isBidirectionalRange!(Unqual!Range))
{
    ...
    else
    {
        ...
        public:
            ...
            {
                @property auto front(ElementType!R val) { source.back = val; }
            }
            void popFront() { source.popBack(); }

            static if (is(typeof(.moveBack(source))))
            {
                ElementType!R moveFront() { return .moveBack(source); }
            }

            @property auto save() { return typeof(this)(source.save); }

            @property auto ref back() { return source.front; }
            static if (hasAssignableElements!R)
            {
                @property auto back(ElementType!R val) { source.front = val; }
            }
            void popBack() { source.popFront(); }

            static if (is(typeof(.moveFront(source))))
            {
                ElementType!R moveBack() { return .moveFront(source); }
            }
            ...
            static if (isRandomAccessRange!R && hasLength!R)
            {
                ...
                {
                    auto opSlice(IndexType a, IndexType b)
                    {
            ...
            static if (hasLength!R)
            {
                @property auto length() { return source.length; }
 ...
 * ----
 */
auto chain(Range)(Range range)
if (isInputRange!(Unqual!Range))
{
    return range;
}
/// ditto
auto chain(Ranges...)(Ranges ranges)
if (Ranges.length > 1 &&
    allSatisfy!(isInputRange, staticMap!(Unqual, Ranges)) &&
    !is(CommonType!(staticMap!(ElementType, staticMap!(Unqual, Ranges))) == void))
{
    alias R = staticMap!(Unqual, Ranges);
    alias E = CommonType!(staticMap!(.ElementType, R));
    template sameET(A) { enum sameET = is(.ElementType!A == E); }
    enum bool allSameType = allSatisfy!(sameET, R);

    static if (allSameType && allSatisfy!(hasLvalueElements, R))
    {
        static ref E fixRef(ref E val) { return val; }
    }
    else
    {
        static E fixRef(E val) { return val; }
    }
...
// TODO: use a vtable (or more) instead of linear iteration
    ...
    public:
        ...
        static if (allSatisfy!(isRandomAccessRange, R))
        {
            ...
            static if (allSameType && allSatisfy!(hasAssignableElements, R))
            {
                void opIndexAssign(E v, size_t index)
                {
                    foreach (i, Range; R)
                    {
                        static if (isInfinite!(Range))
        ...
        static if (allSatisfy!(hasLength, R) && allSatisfy!(hasSlicing, R))
        {
            auto opSlice(size_t begin, size_t end)
            {
                ...
                foreach (i, Unused; R)
                {
                    ...
                    if (len < begin)
                    {
                        result.source[i] = result.source[i][len .. len];
                        begin -= len;
                    }
                    else
                    {
                        result.source[i] = result.source[i][begin .. len];
                        break;
                    }
                }
                auto cut = length;
                ...
                foreach_reverse (i, Unused; R)
                {
                    ...
                    if (cut > len)
                    {
                        result.source[i] = result.source[i][0 .. 0];
                        cut -= len;
                    }
                    else
                    {
                        result.source[i] = result.source[i][0 .. len - cut];
                        break;
                    }
                }
                return result;
...
unittest
{
    int[] a = [ 1, 2, 3 ];
    int[] b = [ 10, 20, 30, 40 ];
    assert(equal(roundRobin(a, b), [ 1, 10, 2, 20, 3, 30, 40 ]));
}
...
if (isInputRange!(Unqual!Range))
{
    ...
    static if ((!isInfinite!R && hasSlicing!R) || is(Range == Take!T, T))
    {
        alias Take = Range;
    }
    else
    {
        struct Take
        {
            ...
            @property auto ref front()
            {
                assert(!empty, "Attempting to fetch the front of an empty " ~ Take.stringof);
                return source.front;
            }

            void popFront()
            {
                assert(!empty, "Attempting to popFront() past the end of a " ~ Take.stringof);
                source.popFront();
                --_maxAvailable;
            }
            ...
            static if (hasMobileElements!R)
            {
                auto moveFront()
                {
                    assert(!empty, "Attempting to move the front of an empty " ~ Take.stringof);
                    return .moveFront(source);
                }
            }
...
/// ditto
Take!Range take(Range)(Range input, size_t n)
if (isInputRange!(Unqual!Range))
{
    alias R = Unqual!Range;
    ...
    else static if (is(Range == Take!T, T))
    {
        // take(take(r, n1), n2)
        return Range(input.source, min(n, input._maxAvailable));
    }
    else
    {
        // Regular take for input ranges
        return Take!Range(input, n);
    }
}
