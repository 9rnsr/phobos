...
{
    static if (meta.all!("isType!A", Types))
        alias TypeSeq = Types;
    else
...
{
    static if (meta.all!("isValue!a", Values))
...
template bindT(alias templat, args...)
{
    template bindT(rest...)
    {
        alias bindT = applyT!(variadicT!templat, args, rest);
    }
}
...
{
    template rbindT(rest...)
    {
        alias rbindT = applyT!(variadicT!templat, rest, args);
    }
}
...
template delayT(alias templat, args...)
{
    template delayT(_...)
    {
        alias delayT = applyT!(variadicT!templat, args);
    }
}
...
{
    template constantT(_...) { alias constantT = E; }
}
...
template constantT()
{
    template constantT(_...) { alias constantT = Seq!(); }
}
...
template notT(alias pred)
{
    template notT(args...)
    {
        enum notT = !applyT!(variadicT!pred, args);
    }
}
...
template composeT(alias template1 = Seq,
                  alias template2 = Seq)
{
    template composeT(args...)
    {
        alias composeT = applyT!(template1, applyT!(template2, args));
    }
}
...
template guardT(alias templat)
{
    template guardT(args...)
    {
        alias guardT = applyT!(templat, args);
    }
}
...
template compiles(templates...)
{
    ...
    {
        enum compiles = __traits(compiles, mapT!(applierT!args, templates));
    }
}
...
{
    template applierT(alias templat)
    {
        alias applierT = applyT!(templat, args);
    }
}
...
template zipWithT(alias fun, seqs...) if (isZippable!seqs)
{
    ...
    {
        alias transverser = _fun!(transverseT!(i, seqs));
    }
...
template intersectionT(seqs...)
{
    template _impl(seqs...)
    {
        alias _impl = reduceT!(composeT!(pack, .intersectionT), seqs).expand;
    }
    template _impl(alias A, alias B)
    {
        alias _impl = intersectionByT!(metaComp, A, B);
    }
    template _impl(alias A)
    {
        alias _impl = setifyT!(A.expand);
    }
    template _impl()
    {
        alias _impl = Seq!();
