    ...
    hasAssignableElements!Range)
{
    ...
    static if (hasElaborateAssign!E)
    {
        ...
        if (p)
            for ( ; !range.empty ; range.popFront() )
                memcpy(addressOf(range.front), p, E.sizeof);
        else
            static if (isDynamicArray!Range)
                memset(range.ptr, 0, range.length * E.sizeof);
            else
                for ( ; !range.empty ; range.popFront() )
                    memset(addressOf(range.front), 0, E.sizeof);
    }
    else
        fill(range, E.init);
}
 ...
 * strategy is (listed from fastest to slowest): $(UL $(LI If $(D ss ==
 ...
 * moves is performed.)  $(LI Otherwise, if $(D ss ==
...
(SwapStrategy ss = SwapStrategy.stable, Range, Offset...)
(Range range, Offset offset)
if (ss == SwapStrategy.stable && isForwardRange!Range && Offset.length >= 1)
{
    import std.exception : enforce;
...
//     return range(begin(r), partitionold!(comp, ss, assignIterB, Range)(r));
// }
